...
public class MyfeedController {
	...
	// 프로필사진 불러오기
	@GetMapping(value = "ProfilePicture")
	public String ProfilePicture(String id) {
		System.out.println("프로필 사진 진입 " + new Date());
		 
		return service.ProfilePicture(id);
	}
	
	// 북마크 피드 불러오기
	@GetMapping(value = "getBookmark")
	public List<FeedDto> getBookmark(String id) {
		System.out.println("getBookmark 진입 " + new Date());
		List<FeedDto> getBookmark = service.getBookmark(id);
		return getBookmark;
	}
	...
	public String sendQnA(QnADto dto){
		System.out.println("문의작성 체크" + new Date());
		
		boolean b = service.sendQnA(dto);
